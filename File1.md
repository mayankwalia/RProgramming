...
# WEEK 3
```R
> c(21,22,24,34)^c(-3,-2,2,3)/c(4,3,2,1)+c(2,1,4,6)%/%c(3,4)*c(2,5,3,6)+max(c(20,140,114,215)^c(1,2))/min(c(6,2,7,15)^c(2,2))
[1] 11556.25 11556.25 11847.25 50866.25
> c(21,22,24,34)^c(-3,-2,2,3)/c(4,3,2,1)+c(2,4,1,6)%/%c(3,4)*c(2,5,3,6)+max(c(20,140,114,215)^c(1,2))/min(c(6,2,7,15)^c(2,2))
[1] 11556.25 11561.25 11844.25 50866.25
> x1=c(23,25,18,24)
> x2=sqrt(x1^2)+x1*6/x1^2-x1**(1/2)+abs(x1)
> x2
[1] 41.46504 45.24000 32.09069 43.35102
> ceiling(prod(c(2,3,8,9))-sum(c(2,22,23,24,151)))-floor(prod(c(2,3,8,9)+round(c(2,22,23,24,151)))
+ )
[1] -15651690
Warning message:
In c(2, 3, 8, 9) + round(c(2, 22, 23, 24, 151)) :
...
ceiling"
> ceiling(prod(c(2,3,8,9))-sum(c(2,22,23,24,151)))-floor(prod(c(2,3,8,9))+round(c(2,22,23,24,151)))
[1] -224 -244 -245 -246 -373
> ceiling(c(4,8,4,5)^-c(3,9)+sqrt(c(36,16,81,64)**c(2,3))*sqrt(c(2,3,4,5)^-c(1,2))+round(c(4,7,8,9)*round(c(7,9))))
[1]  54  85  97 184
> floor(c(5,6,7,8)^c(2,3))-ceiling(c(5,6,7,8)^c(2,3))+ceiling(c(2,3,4,5)^-c(1,-2))-round(c(5,6,7,8)^c(2,3))+floor(c(5,6,7,8)^c(2,3))
[1]  1  9  1 25
> round(prod(c(1,2,1,2)^c(1,2))*sum(c(1,2,1,2)^c(2,3))+prod(c(1,2,1,2)^c(1,2,3,7))+ceiling(c(5,6,7,8)^c(2,3)))
[1]  825 1016  849 1312
> min(c(98,88,44,76)**c(-2,3))/min(c(49,44,22,38)^c(-2,-3))-prod(c(1,2,1,2)^c(1,2))/max(c(9,4,4,9,26)*min(c(4,3,2,6)))+max(c(22,23,24,25)^c(2,3))
[1] 15633.56
> c(5,6,7,8)%/%4-c(64,64,24,24)%/%c(4,3,5,7)+c(153,646,817,788)%%c(71,63)
[1] -4 -4 33 31
> sqrt(c(2,6,7,8)*c(4,2,2,4)+c(2,4,2,4)*c(-2,2)+c(1,2,4,4)**c(9,2,1,4))
[1]  2.236068  4.898979  3.741657 17.204651
> sqrt(c(2,6,7,8)*c(4,2,2,4)+c(2,4,2,4)*c(-2,2)+c(1,2,4,4)**c(9,2,4,1))
[1]  2.236068  4.898979 16.309506  6.633250
> c(1,2,3,4)/c(1,2,2,4)*c(2,1,3,2)**c(1,2,3,3)-c(2,2,32,5)
[1]  0.0 -1.0  8.5  3.0
> c(9,16,6,20)/c(3,4)+c(4,2,4,2)^c(2,3)+c(2,4,8,6)/c(2,2)-c(3,8,2,4)*c(3,1)
[1] 11  6 16 12
...
[3,]   27   28   29
> x=matrix(21:29,3,3,byrow=True)
Error in matrix(21:29, 3, 3, byrow = True) : object 'True' not found
> x=matrix(21:29,3,3,byrow=FALSE)
> x
     [,1] [,2] [,3]
[1,]   21   24   27
[2,]   22   25   28
